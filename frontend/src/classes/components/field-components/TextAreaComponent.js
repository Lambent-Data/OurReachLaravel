...
export default class TextAreaComponent extends FieldComponent {

  constructor(parent, fieldName, label, value, placeholder, allowFeedback = true) {
    const wrapper = $('<div class="text-area-wrapper"></div>');
    super(wrapper, parent, fieldName, label, undefined, placeholder, allowFeedback);
    this.name = "text-area-" + fieldName;
    this.populate();
    this.value = value;
    const dom = this.dom;
    if (!this.value) {
  ...
  populate() {
    ...
    dom.input.on('dblclick', () => {
      if (comp.parent) {
        comp.parent.startEditing(true, true);
      }
      setTimeout(() => comp.dom.input.focus(), 0);
    });
  }
