...
class fields_types
{
    ...
    public static function render_configuration($cfg, $id)
    {
        ...
        foreach ($tabs_cfg as $tab_id => $cfg)
        {
            ...
            foreach ($cfg as $tab_name => $v)
            {
                ...
                {
                    case 'dropdown':
                        if (isset($v['params']['class']) and strstr($v['params']['class'], 'chosen-sortable') and isset($configuration[$v['name']]))
                        {
                            $v['params']['chosen_order'] = (is_array($configuration[$v['name']]) ? implode(',', $configuration[$v['name']]) : $configuration[$v['name']] );
                        }
	        ...
	        <div class="form-group from_group_' . $v['name'] . '">
    ...
    public static function update_items_fields($current_entity_id, $item_id)
    {
        ...
        //get item info
        $item_info_query = db_query("select e.* " . fieldtype_formula::prepare_query_select($current_entity_id, '') . " from app_entity_" . $current_entity_id . " e where e.id='" . db_input($item_id) . "'");
