...
class fieldtype_input_dynamic_mask
{
    ...
    {
        $this->options = array('title' => TEXT_FIELDTYPE_INPUT_DYNAMIC_MASK_TITLE);
    }
    ...
    function get_configuration()
    {
        ...
        $cfg[TEXT_JS_CODE][] = array('title'=>'', 'name'=>'js_code','type'=>'code','params'=>array('class'=>'form-control'));
        $cfg[TEXT_JS_CODE . ' (' . TEXT_EXAMPLE . ')'][] = array('type'=>'html','html'=> '
        ...
        if (strlen($cfg->get('js_code')))
        {
            ...
            $script = '
                <script>
                  jQuery(function($){  
                    try{                    
                        $("#fields_' . $field['id'] . '").inputmask(' . $cfg->get('js_code') . ');               
                    }catch(error){        
                        alert("' . $error_info . '"+`\n`+error)
                    }
                  });
