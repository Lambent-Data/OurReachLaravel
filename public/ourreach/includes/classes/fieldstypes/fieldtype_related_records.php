...
class fieldtype_related_records
{
    ...
    function reports_query($options)
    {
        ...
        if (strlen($filters['filters_values']) > 0)
        {
            ...
            {
                $filters_values = (strlen($filters['filters_values']) ? explode(',', $filters['filters_values']) : [0 => '']);
                ...
                {
                    $where_items_sql = " and ri.entity_" . $cfg->get('entity_id') . "_items_id in (" . implode(',', $filters_values) . ")";
                }
