...
END;";
        ...
        {
            if ($check['Name'] == 'rukovoditel_days_diff')
            {
                $is_function = true;
            }
        }
    ...
    public static function prepare_query($fields, $prefix = 'e', $single_select = false, $force_query = false)
    {
        ...
        if ($single_select)
        {
            $mysql_query = "(rukovoditel_days_diff(" . $start_date_field . "," . $end_date_field . ",'" . $exclude_days . "','" . $cfg->get('exclude_last_day') . "','" . $cfg->get('exclude_holidays') . "'))";
        } else
        {
            $mysql_query = "rukovoditel_days_diff(" . $start_date_field . "," . $end_date_field . ",'" . $exclude_days . "','" . $cfg->get('exclude_last_day') . "','" . $cfg->get('exclude_holidays') . "') as field_" . $fields['id'];
        }
