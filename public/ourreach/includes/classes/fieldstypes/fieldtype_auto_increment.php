...
class fieldtype_auto_increment
{
	...
	function get_configuration()
	{
		...
		$cfg[TEXT_VALUE][] = array('title'=>TEXT_DEFAULT_VALUE, 'name'=>'default_value','type'=>'input','tooltip_icon'=>TEXT_DEFAULT_VALUE_INFO ,'tooltip'=>TEXT_DEFAULT . ': 1','params'=>array('class'=>'form-control input-small number'));
		$cfg[TEXT_VALUE][] = array('title'=>TEXT_STEP,'name'=>'step','type'=>'input','params'=>array('class'=>'form-control input-small'),'tooltip'=>TEXT_DEFAULT . ': ' . 1);
	...
	function increment($field)
	{
	    ...
	    if($app_entities_cache[$field['entities_id']]['parent_id']>0 and  $parent_entity_item_id>0  and $cfg->get('separate_numbering')==1)
	    {
	        $where_sql .= " and parent_item_id='" . $parent_entity_item_id . "'";
	    }
	    
	    $check_query = db_query("select id from app_entity_{$field['entities_id']} where length(field_{$field['id']})>0 {$where_sql} limit 1");
	    ...
	    {	        	        
	        ...
	        if($step<0)
	        {
	            $min_query = db_query("select (min(field_{$field['id']}+0)) as min_value from app_entity_{$field['entities_id']} where length(field_{$field['id']})>0 " . $where_sql);
	            $min = db_fetch_array($min_query);
	            eval('$value = $min["min_value"]' . $step . ';');
	        }
	        else
	        {
	            $max_query = db_query("select (max(field_{$field['id']}+0)) as max_value from app_entity_{$field['entities_id']} where length(field_{$field['id']})>0 " . $where_sql);
