...
class fieldtype_entity
{
    ...
    static function get_choices($field, $params = array(), $value = '', &$parent_entity_item_is_the_same)
    {
    ...
    function render($field, $obj, $params = array())
    {
        ...
        elseif ($cfg->get('display_as') == 'dropdown_muliple')
        {
            ...
            if (strlen($button_add_html))
            {
                $html = '
                <div class="dropdown-with-plus-btn ' . $cfg->get('width') . '">
                    <div class="left">' . select_tag('fields[' . $field['id'] . '][]', $choices, explode(',', $value), $attributes) . '</div>
                    <div class="right">' . $button_add_html . '</div>
                 </div>';
            }
            else
