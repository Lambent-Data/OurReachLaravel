...
class fieldtype_hours_difference
{
    ...
    {
        $this->options = array('title' => TEXT_FIELDTYPE_HOURS_DIFFERENCE_TITLE);
    }
    ...
    function get_configuration()
    {
        ...
        $cfg[] = array('title' => TEXT_WORKING_HOURS, 'name' => 'include_hours', 'tooltip_icon' => TEXT_WORKING_HOURS_INFO, 'default' => '', 'type' => 'dropdown', 'choices' => $choices, 'params' => array('class' => 'form-control input-xlarge chosen-select', 'multiple' => 'multiple'));
    ...
    static function prepare_procedure()
    {
        ...
        $sql = "
CREATE FUNCTION `rukovoditel_hours_diff`(`start_date` INT, `end_date` INT, `include_hours` VARCHAR(255), `exclude_days` VARCHAR(64), `exclude_holidays` TINYINT(1)) RETURNS FLOAT
BEGIN
...
END;";
        ...
        {
            if ($check['Name'] == 'rukovoditel_hours_diff')
            {
                $is_function = true;
            }
        }
    ...
    public static function prepare_query($fields, $prefix = 'e', $single_select = false, $force_query = false)
    {
        ...
        if ($single_select)
        {
            $mysql_query = "(rukovoditel_hours_diff(" . $start_date_field . "," . $end_date_field . ",'" . $include_hours . "','" . $exclude_days . "','" . $cfg->get('exclude_holidays') . "'))";
        } else
        {
            $mysql_query = "rukovoditel_hours_diff(" . $start_date_field . "," . $end_date_field . ",'" . $include_hours . "','" . $exclude_days . "','" . $cfg->get('exclude_holidays') . "') as field_" . $fields['id'];
        }
