...
class fieldtype_months_difference
{
	public $options;

	function __construct()
	{
		$this->options = array('title' => TEXT_FIELDTYPE_MONTHS_DIFFERENCE_TITLE);
	}
	...
	function get_configuration()
	{
		...
		$cfg[] = array('title'=>TEXT_CALCULATE_DIFFERENCE_DAYS, 'tooltip_icon'=>TEXT_CALCULATE_DIFFERENCE_DAYS_INFO,'name'=>'calclulate_diff_days','type'=>'checkbox');
	...
	static function prepare_procedure()
	{
		...
		$sql = "
CREATE FUNCTION `rukovoditel_months_diff`(`start_date` INT, `end_date` INT, `inc_days` TINYINT(1)) RETURNS int(11)
BEGIN  				
  DECLARE months_diff INT;  
  SET months_diff=0;
	
	IF inc_days=1 THEN			
		SET months_diff = TIMESTAMPDIFF(MONTH, DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(0),INTERVAL start_date SECOND),'%Y-%m-%d') , DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(0),INTERVAL end_date SECOND),'%Y-%m-%d') );
	ELSE
		SET months_diff = PERIOD_DIFF(DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(0),INTERVAL end_date SECOND),'%Y%m'),DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(0),INTERVAL start_date SECOND),'%Y%m'));		
	END IF;
...
END;";
		...
		{
			if($check['Name']=='rukovoditel_months_diff')
			{
				$is_function = true;
			}
		}
	...
	public static function prepare_query($fields,$prefix = 'e', $single_select = false, $force_query = false)
	{
		...
		if($single_select)
		{
			$mysql_query =  "(rukovoditel_months_diff(" . $start_date_field . "," . $end_date_field . "," . $cfg->get('calclulate_diff_days',0). "))";
		}
		else
		{
			$mysql_query =  "rukovoditel_months_diff(" . $start_date_field . "," . $end_date_field . "," . $cfg->get('calclulate_diff_days',0). ") as field_" . $fields['id'];
		}
