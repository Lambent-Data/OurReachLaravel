<?php
  ...
  function button_tag($value,$url,$is_dialog = true,$attributes=array(),$left_icon='', $right_icon='')
  {
    ...
    if(strlen($left_icon)>0) $left_icon = app_render_icon($left_icon) . ' ';
    if(strlen($right_icon)>0) $right_icon = ' ' . app_render_icon($right_icon);
  ...
  function input_color($name, $color='')
  {
      $html = '
          <div class="input-group input-small color colorpicker-default" data-color="' . (strlen($color) ? $color:'#cccccc') . '" >
                  <span class="input-group-btn"><button class="btn btn-default" type="button"><i></i>&nbsp;</button></span>
                  ' . input_tag($name,$color,array('class'=>'form-control','style'=>'width: 80px')) .'                    
          </div>
         ';
