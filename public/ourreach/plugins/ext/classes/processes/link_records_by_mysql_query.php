...
class link_records_by_mysql_query
{
    ...
    function process($sql_data, $choices_values)
    {
        ...
        $item_info_query = db_query("select * from app_entity_" . $this->current_entity_id . " where id='" . db_input($this->current_item_id) . "'");
        ...
        {            
            if(preg_match_all('/\[(\w+)\]/',$this->where_query,$matches))
            {
                ...
                {                                         
                    ...
                    {
                        $this->where_query = str_replace('[' . $fields_id . ']',$item_info['field_' . $fields_id],$this->where_query);
                    }
                    
                    if(isset($item_info[$fields_id]))
                    {
                        $this->where_query = str_replace('[' . $fields_id . ']',$item_info[$fields_id],$this->where_query);
                    }
                }
            }
        }
        ...
        $items_query = db_query("select e.id " .  fieldtype_formula::prepare_query_select($this->action_entity_id,'') . " from app_entity_{$this->action_entity_id} e where " . $this->where_query);
        while($items = db_fetch_array($items_query))
        {
            ...
            $check_query = db_query("select id from " . $table_info['table_name'] . " where entity_" . $this->current_entity_id. "_items_id=" . $this->current_item_id . " and entity_" . $this->action_entity_id . $table_info['sufix'] . "_items_id=" . $items['id']);
