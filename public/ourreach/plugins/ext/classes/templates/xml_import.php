...
class xml_import
{
    ...
    function has_xml_errors()
    {
        ...
        if(count($errors))
        {
            ...
            {
                $html .= '<li>' . sprintf(TEXT_EXT_ERROR_ON_LINE_COLUMN,$error->line,$error->column) . ': <b>'. $error->message . '</b></li>';
            }
    ...
    {               
        ...
        $import_fields = (strlen($this->template_info['import_fields']) ? json_decode($this->template_info['import_fields'],true):[]);
        $import_fields_path = (strlen($this->template_info['import_fields_path']) ? json_decode($this->template_info['import_fields_path'],true):[]);
        ...
        if($this->template_info['update_by_field']>0 and strlen($this->template_info['update_by_field_path']))
        {
            ...
            {
                $import_fields_queries[$this->template_info['update_by_field']] = str_replace(['[',']'],'',$xml_field_path);
            }
            else
            {
                $import_fields_queries[$this->template_info['update_by_field']] = $xpath->query($xml_data_path  . '/' . $xml_field_path);
            }
        }
    ...
    function prepare_sql_data($item_key,$import_fields_queries,$item_node)
    {
        ...
        {               
            ...
            if($this->template_info['update_by_field']==$field_id and $app_fields_cache[$entities_id][$field_id]['type']=='fieldtype_id') continue;
            ...
            {                
                ...
                switch($filed_info['type'])
                {
                    ...
                    case 'fieldtype_input_datetime':    
                        $xml_field_value = (is_array($xml_field_value) ? implode(' ',$xml_field_value) : $xml_field_value);
                        $sql_data['field_' . $field_id] = (is_string($xml_field_value) ? strtotime($xml_field_value) :'');                       
                        break;
                    default:
                        $sql_data['field_' . $field_id] = (is_array($xml_field_value) ? implode(', ',$xml_field_value) : $xml_field_value);
                        break;
                }
        ...
        if($this->template_info['import_action']=='update' or $this->template_info['import_action']=='update_import')
        {
            ...
            {
                $xml_field_value = $this->get_field_value_by_key($this->template_info['update_by_field'],$item_key,$import_fields_queries);
            }
            else
            {
                $xml_field_value = $item_node->getAttribute($import_fields_queries[$this->template_info['update_by_field']]);
            }
    ...
    static function get_users_templates_by_position($entities_id,$position, $url_params='')
    {
        ...
        while($templates = db_fetch_array($templates_query))
        {
            ...
            {
                case 'default':
                    $html .= '<li>' . button_tag($button_title,url_for('items/xml_import','path=' . $_GET['path'] . '&templates_id=' . $templates['id']),true,array('class'=>'btn btn-primary btn-sm btn-template-' . $templates['id']),$button_icon). '</li>';
                    $html .= self::prepare_button_css($templates);
                    break;
                case 'in_listing':
                    $html .= '&nbsp;&nbsp;' . button_tag($button_title,url_for('items/xml_import','path=' . $_GET['path'] . '&templates_id=' . $templates['id']),true,array('class'=>'btn btn-primary btn-template-' . $templates['id']),$button_icon);
                    $html .= self::prepare_button_css($templates);
                    break;             
    ...
    static function has_users_access($entities_id, $templates_id)
    {
        ...
        $templates_query = db_query("select ep.* from app_ext_xml_import_templates ep, app_entities e where e.id=ep.entities_id and ep.is_active=1 and ep.entities_id='" . db_input($entities_id) . "' and (find_in_set(" . db_input($app_user['group_id']) . ",users_groups) or find_in_set(" . db_input($app_user['id']) . ",assigned_to)) and ep.id='" . db_input($templates_id) . "' order by ep.sort_order, ep.name");
