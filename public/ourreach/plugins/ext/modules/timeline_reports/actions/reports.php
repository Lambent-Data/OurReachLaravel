...
switch($app_module_action)
{
	...
	case 'delete':
		...
		$report_info_query = db_query("select * from app_reports where reports_type='timelinereport" . db_input($_GET['id']). "'");
	...
	case 'get_entities_fields':
			...
			$fields_query = db_query("select * from app_fields where type in ('fieldtype_input_date','fieldtype_input_datetime','fieldtype_date_added','fieldtype_dynamic_date') and entities_id='" . db_input($entities_id) . "'");
        ...
        ';
			...
			$use_fields = array();
			$use_fields[''] = '';
			$fields_query = db_query("select * from app_fields where type in ('fieldtype_dropdown','fieldtype_radioboxes','fieldtype_autostatus','fieldtype_stages','fieldtype_grouped_users') and entities_id='" . db_input($entities_id) . "'");
			while($fields = db_fetch_array($fields_query))
			{
