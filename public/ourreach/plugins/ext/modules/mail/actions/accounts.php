...
switch ($app_module_action)
{
    ...
    case 'search_contacts':
        ...
        if (strlen($_GET['q']) > 1)
        {
            $contacts_query = db_query("select * from app_ext_mail_contacts where name like '%" . db_input($_GET['q']) . "%' or email like '%" . db_input($_GET['q']) . "%' and accounts_id in (select accounts_id from app_ext_mail_accounts_users where users_id='" . $app_user['id'] . "') limit 10");
    ...
    case 'empty_trash':
        //delete attachments
        $mail_query = db_query("select attachments from app_ext_mail where in_trash=1 and length(attachments)>0 " . ($app_mail_filters['accounts_id'] > 0 ? " and accounts_id='" . $app_mail_filters['accounts_id'] . "'" : ''));
        ...
        //delete rows
        db_query("delete from app_ext_mail where in_trash=1 " . ($app_mail_filters['accounts_id'] > 0 ? " and accounts_id='" . $app_mail_filters['accounts_id'] . "'" : ''));

        //reset mail groups
        $groups_list = [];
        $groups_query = db_query("select mg.id from app_ext_mail_groups mg where (select count(*) from app_ext_mail m where m.groups_id=mg.id)=0");
        ...
        if (count($groups_list))
        {
            ...
            db_query("delete from app_ext_mail_groups_from where mail_groups_id in (" . implode(',', $groups_list) . ")");
    ...
    case 'attachments_upload':
        ...
        if (strlen($_FILES['Filedata']['tmp_name']))
        {
            ...
            if (move_uploaded_file($_FILES['Filedata']['tmp_name'], DIR_WS_MAIL_ATTACHMENTS . $file['folder'] . '/' . $file['file']))
            {
                //add attachments to tmp table
                $sql_data = array('form_token' => $_POST['token'], 'filename' => $file['name'], 'date_added' => date('Y-m-d'), 'container' => 0);
    ...
    case 'attachment_delete':
        $attachments_query = db_query("select * from app_attachments where id='" . db_input($_POST['id']) . "' and container=0");
    ...
    case 'delete_mails':
        ...
        {
            db_query("update app_ext_mail set in_trash=1 where groups_id in (" . implode(',',$_POST['selected_items']). ")");           
        }
    ...
    case 'update_notifications':
        ...
        while ($items = db_fetch_array($items_query))
        {
          ...
          <li>
  					...
  					<span class="parent-name">' . (strlen($items['from_name']) ? $items['from_name'] : $items['from_email'] ) . ' ' . ($items['count_mails'] > 1 ? $items['count_mails'] : '') . '</span></a>
  				</li>
        ';
        ...
        if ($items_count > 0)
        {
            $external_html = '
          <li class="external">
						<a href="' . url_for('ext/mail/accounts') . '">' . sprintf(TEXT_EXT_DISPLAY_NUMBER_OF_MAIL_INBOX, $items_display_count) . '</a>
					</li>
        ';
        }
        ...
        <a href="#" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">
				...
				<ul class="dropdown-menu extended tasks">
					<li style="cursor:pointer" onClick="location.href=\'' . url_for('ext/mail/accounts', 'action=set_folder&folder=inbox') . '\'">
