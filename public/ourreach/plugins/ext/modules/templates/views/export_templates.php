...
<tr>
  <td style="white-space: nowrap;"><?php 
  	echo button_icon_delete(url_for('ext/templates/export_templates_delete_confirm','id=' . $templates['id'])) . ' ' . 
  	button_icon_edit(url_for('ext/templates/export_templates_form','id=' . $templates['id'])) . ' ' . 
  	button_icon(TEXT_BUTTON_CONFIGURE_FILTERS,'fa fa-cogs',url_for('ext/templates/export_templates_filters','templates_id=' . $templates['id']),false) . ' '. 
  	button_icon(TEXT_COPY,'fa fa-files-o',url_for('ext/templates/export_templates','action=copy&templates_id=' . $templates['id']),false,['onClick'=>'return confirm("' . addslashes(TEXT_COPY_RECORD). '?")'])
  	?></td>
  ...
  {
      echo link_to($templates['name'],url_for('ext/templates/export_templates_configure','id=' . $templates['id']));
  }
  else
  {
      $html = link_to($templates['name'],url_for('ext/templates_docx/blocks','templates_id=' . $templates['id']));
