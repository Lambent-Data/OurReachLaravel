...
<?php echo ajax_modal_template_header(TEXT_EXT_GRAPHIC_REPORT) ?>

<?php echo form_tag('configuration_form', url_for('ext/graphicreport/configuration','action=save' . (isset($_GET['id']) ? '&id=' . $_GET['id']:'') ),array('class'=>'form-horizontal')) ?>
...
<div class="tab-content">
  <div class="tab-pane fade active in" id="general_info">
    ...
    <div class="form-group">
    	<label class="col-md-3 control-label" for="type"><?php echo TEXT_EXT_CHART_TYPE ?></label>
      <div class="col-md-9">	
    	  <?php echo select_tag('chart_type',array('line'=>TEXT_EXT_CHART_TYPE_LINE,'column'=>TEXT_EXT_CHART_TYPE_COLUMN), $obj['chart_type'],array('class'=>'form-control input-large required')) ?>        
      </div>			
    </div>  
    
    <div class="form-group">
    	<label class="col-md-3 control-label" for="type"><?php echo TEXT_PERIOD ?></label>
      <div class="col-md-9">	
    	  <?php echo select_tag('period',array('daily'=>TEXT_DAILY,'monthly'=>TEXT_MONTHLY,'yearly'=>TEXT_YEARLY), $obj['period'],array('class'=>'form-control input-large required')) ?>        
      </div>			
    </div>
...
{ 
  ...
  $('#reports_entities_fields').load('<?php echo url_for("ext/graphicreport/configuration","action=get_entities_fields")?>',{entities_id:entities_id, id:'<?php echo $obj["id"] ?>'},function(response, status, xhr) {
    if (status == "error") {                                 
