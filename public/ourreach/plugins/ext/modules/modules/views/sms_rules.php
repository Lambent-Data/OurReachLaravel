...
$rules_query = db_query("select r.*, e.name as entity_name, m.module from app_ext_sms_rules r left join app_ext_modules m on m.id=r.modules_id, app_entities e where e.id=r.entities_id order by e.name, r.action_type");
...
?>
<tr>
  <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('ext/modules/sms_rules_delete','id=' . $rules['id'])) . ' ' . button_icon_edit(url_for('ext/modules/sms_rules_form','id=' . $rules['id'])) ?></td>
  <td><?php echo $module_title ?></td>
  <td><?php echo $rules['entity_name'] ?></td>
  <td><?php echo (strstr($rules['action_type'],'insert') ? '<span class="label label-success">' . TEXT_EXT_ADDING_NEW_RECORD .'</label>': '<span class="label label-info">' . TEXT_EXT_ADDITING_RECORD .'</span>') ?></td>
  <td><?php echo sms::get_action_type_name($rules['action_type']) . ($rules['monitor_fields_id']>0 ? '<br><i>' . TEXT_EXT_PB_NOTIFY_FIELD_CHANGE . ':</i> <span class="label label-warning">' . $fields_cahce[$rules['monitor_fields_id']]  . '</span>': '') ?></td>  
  <td><?php 
   ...
   ?></td>    
  <td><?php echo nl2br($rules['description']) ?></td>
</tr>  
<?php endwhile ?>
</tbody>
</table>
