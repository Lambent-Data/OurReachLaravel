...
<?php echo form_tag('configuration_form', url_for('ext/map_reports/reports','action=save' . (isset($_GET['id']) ? '&id=' . $_GET['id']:'') ),array('class'=>'form-horizontal')) ?>
...
{ 
  ...
  $('#reports_entities_fields').load('<?php echo url_for("ext/map_reports/reports","action=get_entities_fields")?>',{entities_id:entities_id, id:'<?php echo $obj["id"] ?>'},function(response, status, xhr) {
    if (status == "error") {                                 
