<?php 
  ...
  $breadcrumb[] = '<li>' . link_to($reports['name'],url_for('ext/pivotreports/view','id=' . $_GET['id'])) . '<i class="fa fa-angle-right"></i></li>';
    
  $breadcrumb[] = '<li>' . link_to(TEXT_EXT_PIVOTREPORTS_FIELDS,url_for('ext/pivotreports/fields','id=' . $_GET['id'])) . '</li>';
...
<?php 
	...
	$pivotreports_fields_query = db_query("select * from app_ext_pivotreports_fields where pivotreports_id='" . db_input($_GET['id']) . "'");
		...
		while($fields = db_fetch_array($fields_query))
		{
			$html .= '
				<tr>
					...
					<td>' . (in_array($fields['type'],array('fieldtype_date_added','fieldtype_input_date','fieldtype_input_datetime')) ? 
