...
<?php
  ...
  $templates_query = db_query("select ep.*, e.name as entities_name from app_ext_xml_export_templates ep, app_entities e where e.id=ep.entities_id " . $where_sql . " order by e.id, ep.sort_order, ep.name");
?>  
...
?>
<tr>
  <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('ext/xml_export/delete_confirm','id=' . $templates['id'])) . ' ' . button_icon_edit(url_for('ext/xml_export/form','id=' . $templates['id'])) ?></td>
  <td><?php echo $templates['entities_name'] ?></td>
  <td style="white-space: normal"><?php echo $templates['name'] . 
								 '<div><small><a href="' . url_for('ext/xml_export/filters','templates_id=' . $templates['id']) . '">' . TEXT_FILTERS . ' (' . xml_export::count_filters($templates['id']). ')</a></small></div>';  					
