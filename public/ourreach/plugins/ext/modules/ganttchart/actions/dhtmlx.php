...
$reports_info_query = db_query("select * from app_reports where entities_id='" . db_input($reports['entities_id']). "' and reports_type='{$reports_type}' and created_by='" . $app_logged_users_id . "'");
...
{
	case 'set_grid_width':
		db_query("update app_ext_ganttchart set grid_width='" . _post::int('grid_width'). "' where id='" . $reports['id'] . "'");
		exit();
		break;
	case 'save':
			...
			case 'tasks':
				...
				switch($_POST['!nativeeditor_status'])
				{
					case 'order':
						...
						db_perform("app_entity_" . $reports['entities_id'], $sql_data,'update',"id='" . $_POST['id'] . "'");
						...
						if($next_task_id)
						{
							...
							$item_info_query = db_query("select sort_order from app_entity_" . $reports['entities_id'] . " where id='" . $next_task_id . "'");
							$item_info = db_fetch_array($item_info_query);
							
							//set next sort order to current item
							db_query("update app_entity_" . $reports['entities_id'] . " set sort_order='" . $item_info['sort_order']. "' where id='" . $_POST['id'] . "'");
							
							//update all other sort orders in current parent
							db_query("update app_entity_" . $reports['entities_id'] . " set sort_order=sort_order+1 where (sort_order>'" . $item_info['sort_order'] . "' or id='" . $next_task_id . "') and parent_id='" . $_POST['parent'] . "'");
						}
						else
						{
							$max_info_query = db_query("select max(sort_order) as value from app_entity_" . $reports['entities_id'] . " where parent_id='" . $_POST['parent'] . "'");
							$max_info = db_fetch_array($max_info_query);
							
							//set next sort order to current item
							db_query("update app_entity_" . $reports['entities_id'] . " set sort_order='" . ($max_info['value']+1) . "' where id='" . $_POST['id'] . "'");
						}
					...
					case 'updated':
						...
						$sql_data = array(
							...
							'field_' . $reports['end_date'] => strtotime("-1 day",get_date_timestamp($_POST['end_date'])),							
						);
					...
					case 'inserted':
						...
						//handle sort order
						$max_sort_order_query = db_query("select max(sort_order) as value from app_entity_" . $reports['entities_id'] . " where parent_id='" . (int)$_POST['parent'] . "'");
						$max_sort_order = db_fetch_array($max_sort_order_query);
						$sql_data['sort_order'] = ($max_sort_order['value']+1);
						
						db_perform("app_entity_" . $reports['entities_id'], $sql_data,'update',"id='" . $item_id . "'");
					...
					case 'deleted':
						...
						db_query("delete from app_ext_ganttchart_depends where (item_id='" . $_POST['id'] . "' or depends_id='" . $_POST['id'] . "') and ganttchart_id='" . $reports['id'] . "' and entities_id='" . $reports['entities_id'] . "'");
						break;
				}
	...
	case 'load_data':
		...
		{
		    $listing_sql_query_select .= ", (select min(parent.field_{$reports['start_date']}) from app_entity_{$reports['entities_id']} as parent where parent.parent_item_id=e.parent_item_id) as parent_min_start_date";
		    $listing_order_query = " parent_min_start_date, ";
		    
		    $display_parent_item = true;
		}
								
		$listing_sql = "select e.* " . $listing_sql_query_select . " from app_entity_" . $reports['entities_id'] . " e  where  e.id>0 " . $listing_sql_query . " order by " . $listing_order_query . " e.sort_order, e.field_" . $reports['start_date'] . "";
		...
		while($item = db_fetch_array($items_query))
		{
		    ...
		    if($display_parent_item)
		    {
		        if(!in_array($item['parent_item_id'],$parent_item_list))
		        {
    		        ...
    		        $row = array(
    		            ...
    		            'text' => '<a href="' . url_for('ext/ganttchart/dhtmlx','id=' . $reports['id'] . '&path=' . $path_info['path_to_entity']) . '" target="_blank"><b>' . $path_info['parent_name'] . '</b></a>',
    		            'start_date' => '',
    		            'duration' => '',
    		            'progress' => '',
    		            'parent' => 0,
			...
			{
				db_query("update app_entity_" . $reports['entities_id'] . " set sort_order='" . $sort_order . "' where id='" . $item['id'] . "'");
			}	
			...
			$row += array(
					'open' => true,
					'id' => $item['id'],
					'text' => '<a ' . $heading_link_color . ' href="' . url_for('items/info','path=' . $path_info['full_path']) . '" target="_blank">' . $heading_field_value . '</a>',
		...
		$depends_query = db_query("select * from app_ext_ganttchart_depends where ganttchart_id='" . db_input($reports['id']). "'");
