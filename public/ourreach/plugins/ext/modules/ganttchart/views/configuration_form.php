...
<?php 
	$choices = ['day'=>TEXT_EXT_DAY,'week'=>TEXT_EXT_WEEK,'month'=>TEXT_EXT_MONTH,'year'=>TEXT_EXT_YEAR];
?>    
		<div class="form-group">
    	<label class="col-md-3 control-label" for="type"><?php echo TEXT_EXT_DEFAULT_VIEW ?></label>
      <div class="col-md-9">	
    	  <?php echo select_tag('default_view',$choices, $obj['default_view'],array('class'=>'form-control input-medium ')) ?>        
      </div>			
    </div>  
    
    
<?php 
	$choices = ['terrace'=>'Terrace','skyblue'=>'Skyblue','meadow'=>'Meadow','broadway'=>'Broadway','material'=>'Material','contrast_white'=>'High contrast light'];
?>     
    <div class="form-group">
    	<label class="col-md-3 control-label" for="skin"><?php echo TEXT_SKIN ?></label>
      <div class="col-md-9">	
    	  <?php echo select_tag('skin',$choices,$obj['skin'],array('class'=>'form-control input-medium')) ?>               
      </div>			
    </div>
...
?>    
    <div class="form-group">
    	<label class="col-md-3 control-label" for="type"><?php echo TEXT_DISPLAY_FIELDS ?></label>
      <div class="col-md-9">	
    	  <?php echo select_tag('default_fields_in_listing[]',$choices, $obj['default_fields_in_listing'],array('class'=>'form-control input-xlarge chosen-select','multiple'=>'multiple')) ?>        
      </div>			
    </div>
    ...
    <div class="form-group">
    	<label class="col-md-3 control-label" for="name"><?php echo TEXT_EXT_GRID_WIDTH ?></label>
      <div class="col-md-9">	
    	  <?php echo input_tag('grid_width',$obj['grid_width'],array('class'=>'form-control input-small')) ?>        
      </div>			
    </div>
...
?>  
  ...
  <div class="tab-pane fade" id="dhtmlxGanttPRO">
  	<p><?php echo TEXT_EXT_DHTMLXGANTT_PRO_INFO ?></p>
  	
  	<div class="form-group">
    	<label class="col-md-4 control-label" >dhtmlxGantt PRO</label>
      	<div class="col-md-8">	
  			<p class="form-control-static"><?php echo app_render_status_label(is_file('js/dhtmlxGantt/Pro/codebase/dhtmlxgantt.js') ? true:false) ?></p>  	   
      	</div>			
    </div>
    
    <div class="form-group">
    	<label class="col-md-4 control-label" for="auto_scheduling"><?php echo TEXT_EXT_AUTO_SCHEDULING ?></label>
      	<div class="col-md-8">	  			
  			<?php echo select_tag('auto_scheduling',['0'=>TEXT_NO,'1'=>TEXT_YES],$obj['auto_scheduling'],['class'=>'form-control input-small']) ?>
  			<?php echo tooltip_text(TEXT_EXT_AUTO_SCHEDULING_INFO) ?>  	   
      	</div>			
    </div>
    
    <div class="form-group">
    	<label class="col-md-4 control-label" for="highlight_critical_path"><?php echo TEXT_EXT_HIGHLIGHT_CRITICAL_PATH ?></label>
      	<div class="col-md-8">	  			
  			<?php echo select_tag('highlight_critical_path',['0'=>TEXT_NO,'1'=>TEXT_YES],$obj['highlight_critical_path'],['class'=>'form-control input-small']) ?>
  			<?php echo tooltip_text(TEXT_EXT_HIGHLIGHT_CRITICAL_PATH_INFO) ?>  	   
      	</div>			
    </div>
...
{
	$('#entity_listing_fields').load('<?php echo url_for("ext/ganttchart/configuration","action=get_entity_listing_fields")?>',{entities_id:$('#entities_id').val(), id:'<?php echo $obj["id"] ?>'},function(response, status, xhr) {
    if (status == "error") {                                 
       $(this).html('<div class="alert alert-error"><b>Error:</b> ' + xhr.status + ' ' + xhr.statusText+'<div>'+response +'</div></div>')                    
    }
    else
