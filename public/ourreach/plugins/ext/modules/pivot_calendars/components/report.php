...
<div id="calendar<?php echo $reports['id'] ?>"></div>
  ...
  var allow_add_item = 1;
	...
	$(document).ready(function() {
      ...
      eventResize: function(event, delta, revertFunc) {
        $.ajax({type: "POST",url: "<?php echo url_for('ext/pivot_calendars/view','action=resize&id=' . $reports['id'])?>",data: {id:event.id,entities_id:event.entities_id,end:event.end.format()}});
      },
      eventDrop: function(event, delta, revertFunc) {
        if(event.end)
        {
          $.ajax({type: "POST",url: "<?php echo url_for('ext/pivot_calendars/view','action=drop&id=' . $reports['id'])?>",data: {id:event.id,entities_id:event.entities_id,start:event.start.format(),end:event.end.format()}});
        }
        else
        {
          $.ajax({type: "POST",url: "<?php echo url_for('ext/pivot_calendars/view','action=drop&id=' . $reports['id'])?>",data: {id:event.id,entities_id:event.entities_id,start:event.start.format()}});
        }
      ...
      eventMouseout:function(calEvent, jsEvent, view) {
        $(this).popover('hide');
			...
			events: {
				url: '<?php echo url_for("ext/pivot_calendars/view","action=get_events&mode=full&id=" . $reports["id"])?>',
        error: function() {
	...
	$(document).ready(function() {
      ...
      eventMouseout:function(calEvent, jsEvent, view) {
        $(this).popover('hide');
			...
			events: {
				url: '<?php echo url_for("ext/pivot_calendars/view","action=get_events&mode=view&id=" . $reports["id"])?>',
        error: function() {
				  alert('<?php echo TEXT_ERROR_LOADING_DATA ?>')
				}				
			},
