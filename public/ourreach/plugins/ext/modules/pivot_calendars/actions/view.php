...
{	
	case 'resize':
		...
		$reports_entities_query = db_query("select ce.*, e.name from app_ext_pivot_calendars_entities ce, app_entities e where e.id=ce.entities_id and ce.calendars_id='" . $reports['id'] . "' and ce.entities_id='" . $entities_id . "'");
	...
	case 'get_events':
		...
		{				
			if(!users::has_users_access_name_to_entity('view',$reports_entities['entities_id'])) continue;
			...
			$listing_sql_query = reports::add_filters_query(pivot_calendars::get_reports_id_by_calendar_entity($reports_entities['id'],$reports_entities['entities_id']),$listing_sql_query);
			...
			{
			    $sql_query_having[$reports_entities['entities_id']][] = substr(str_replace('e.','',$listing_sql_query_filter),4);
			}
			
			//prepare having query for formula fields
			if(isset($sql_query_having[$reports_entities['entities_id']]))
			{
				$listing_sql_query_having  = reports::prepare_filters_having_query($sql_query_having[$reports_entities['entities_id']]);
			}
			...
			$events_query = db_query("select e.* "  . $listing_sql_query_select . " from app_entity_" . $reports_entities['entities_id'] . " e where e.id>0 " . $listing_sql_query);
			while($events = db_fetch_array($events_query))
			{
				...
				{
					$options = array('custom_pattern' => $reports_entities['heading_template'],
							'item' => $events);
				...
				$color = (strlen($reports_entities['bg_color']) ? $reports_entities['bg_color'] : '#3a87ad') . ' !important';
				if($reports_entities['use_background']>0)
				{
				...
				if(strlen($reports_entities['fields_in_popup']))
				{
					...
					foreach(explode(',',$reports_entities['fields_in_popup']) as $fields_id)
					{
						...
						if($field = db_fetch_array($field_query))
						{
							...
							if(strlen($value)>255) $value = mb_substr($value,0,255) . '...';
