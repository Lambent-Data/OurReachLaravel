  ...
  <td>
  	<?php 
  	    ...
  	    echo '<small>&nbsp;&nbsp;' . link_to(TEXT_EXT_PIVOTREPORTS_FIELDS,url_for('ext/pivot_tables/fields','id=' . $reports['id'])) . '</small>';
            ...
            {
                echo '<small>&nbsp;|&nbsp;' . link_to(TEXT_DEFAULT_FILTERS,url_for('default_filters/filters','reports_id=' . default_filters::get_reports_id($reports['entities_id'], 'default_pivot_tables' . $reports['id']) . '&redirect_to=pivot_tables' . $reports['id'])) . '</small>';
            }
            
            if($reports['filters_panel']=='quick_filters')
            {
                echo '<small>&nbsp;|&nbsp;' . TEXT_QUICK_FILTERS_PANELS . ': <a href="' . url_for('ext/filters_panels/fields','panels_id=' . filters_panels::get_id_by_type($reports['entities_id'], 'pivot_tables' . $reports['id']) . '&entities_id=' . $reports['entities_id'] . '&redirect_to=pivot_tables' . $reports['id']) . '">' . TEXT_CONFIGURE . '</a></small></small>';
            }
  	?>
  </td>  
