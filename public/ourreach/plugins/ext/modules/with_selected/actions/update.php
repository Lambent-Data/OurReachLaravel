  ...
  case 'get_field_values':
      ...
      switch($field_info['type'])
      {
      	...
      	case 'fieldtype_entity_ajax':
      		...
      		{
      			$listing_sql_query = fieldtype_entity::prepare_parents_sql($parent_entity_item_id,$entity_info['parent_id'],$field_entity_info['parent_id']);
      		}
      		
      		$default_reports_query = db_query("select * from app_reports where entities_id='" . db_input($field_cfg->get('entity_id')). "' and reports_type='entityfield" . $field_info['id'] . "'");
      		...
      		$field_heading_id = 0;
      		$fields_query = db_query("select f.* from app_fields f where f.is_heading=1 and  f.entities_id='" . db_input($field_cfg->get('entity_id')) . "'");
      		if($fields = db_fetch_array($fields_query))
      		{
      			$field_heading_id = $fields['id'];
      		}
      		
      		$listing_sql = "select  e.* from app_entity_" . $field_cfg->get('entity_id') . " e "  . $listing_sql_query_join . " where e.id>0 " . $listing_sql_query;
        ...
        case 'fieldtype_boolean':
            ...
            $html = select_tag('values[' . $fields_id . ']',$choices,'',array('class'=>'form-control input-small'));
          break;
        ...
        case 'fieldtype_stages':
            ...
            $html = select_tag('values[' . $fields_id . ']',$choices,'',array('class'=>'form-control input-large chosen-select'));                
          break;
        ...
        case 'fieldtype_progress':
            ...
            $html = select_tag('values[' . $fields_id . ']',$choices,'',array('class'=>'form-control chosen-select'));
            break;
        ...
        case 'fieldtype_tags':
            ...
            $html = select_tag('values[' . $fields_id . '][]',$choices,'',array('class'=>'form-control chosen-select','multiple'=>'multiple'));
          break;          
          
        case 'fieldtype_input_numeric':
            $html =  input_tag('values[' . $fields_id . ']','',array('class'=>'form-control input-medium')) . tooltip_text(TEXT_EXT_UPDATE_FIELD_INPUT_TIP);
            $html .= '
                <script>
        ...
        case 'fieldtype_input_datetime':
            $html = '
              <div class="input-group input-medium ">' . 
                input_tag('values[' . $fields_id . ']','',array('class'=>'form-control date-fields')) .                 
        ...
        case 'fieldtype_users':
            ...
            {
              $users_query_assigned_only = " and u.id in (select cv.value from app_entity_" . $field_info['entities_id'] . "_values cv where cv.fields_id='" . db_input($field_info['id']) . "' and cv.items_id in (select cvi.items_id from app_entity_" . $field_info['entities_id'] . "_values cvi where cvi.fields_id='" . db_input($field_info['id']) . "' and cvi.value='" . db_input($app_user['id']) . "'))";        
            }
                  
            $choices = array();
            $users_query = db_query("select u.*,a.name as group_name from app_entity_1 u left join app_access_groups a on a.id=u.field_6 where u.field_5=1 " . $users_query_assigned_only . " order by u.field_8, u.field_7");
      ...
      {    
        ...
        foreach($app_selected_items[$_GET['reports_id']] as $item_id)
        {
          ...
          {            
            ...
            //update field value
            db_query("update app_entity_" . $reports_info['entities_id'] . " set field_" . $fields_id . " = '" . db_input($value) . "' where id='" . db_input($item_id) . "'");
            
            //set date updated
            db_query("update app_entity_" . $reports_info['entities_id'] . " set date_updated = '" . time() . "' where id='" . db_input($item_id) . "'");
