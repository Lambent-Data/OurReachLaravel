...
<style>
  div.entitly-listing-buttons-left > *:not(#ld-button) {
    display:none;
  }
  div.entitly-listing-buttons-left > #ld-button {
...
<!-- LAMBENT DATA ADDITIONS -->

<script type="text/javascript">
$(document).ready(()=>{
  const btn = $('<button id="ld-button" type="button" class="btn btn-primary">Add Milestone</button>');
	if (<?php echo (intval($app_user['fields']['field_6']) === 6 ? "true" : "false");  ?>){
		$('div.entitly-listing-buttons-left').prepend(btn);
	}

	//<?php echo intval($app_user['fields']['field_6']); ?>

  btn.click(()=> {
    window.location.href = "http://demo.ourreachld.com/lambent/new_milestone.html?uid=<?php echo $app_user['id'] ?><?php if($app_user['fields']['field_1098']) { echo '&mid='.$app_user['fields']['field_1098']; } ?>";
  });
