...
{  
  case 'save':
      ...
      {      
        $is_new_item = false;          
        $item_info_query = db_query("select * from app_entity_" . $current_entity_id . " where id='" . db_input(_get::int('id')) . "'");
      ...
      $fields_query = db_query("select f.* from app_fields f where f.type not in (" . fields_types::get_reserverd_types_list(). ",'fieldtype_related_records','fieldtype_user_last_login_date','fieldtype_google_map','fieldtype_google_map_directions','fieldtype_php_code') and  f.entities_id='" . db_input($current_entity_id) . "' order by f.sort_order, f.name");
      ...
      {            	      	
        ...
        db_perform('app_entity_' . $current_entity_id,$sql_data,'update',"id='" . db_input(_get::int('id')) . "'");
        $item_id = (int)$_GET['id'];   
  ...
  case 'delete':
      ...
      $item_info_query = db_query("select created_by from app_entity_" . $current_entity_id . " where id='" . $item_id . "'");
 ...
 case 'attachments_delete_in_queue':
  ...
  case 'set_listing_type':
  	...
  	{
  		db_query("update app_reports set listing_type='" . db_input($_GET['type']) . "' where id='" . $reports_info['id'] . "'");
  	}
...
if(!filters_panels::has_any($current_entity_id, $entity_cfg))
{
