...
<?php echo form_tag('dashboard', url_for('dashboard/reports_groups','action=save&id=' . _get::int('id'))) ?>
...
<div class="cfg_listing">        
  ...
  <?php
  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id  and r.reports_type='common' and r.id in (" . $reports_list . ") order by field(r.id,{$reports_list})");
  ...
  ?> 
    ...
    <td style="padding-left: 25px;" valign="top">
      ...
      <fieldset>
        <legend><?php echo TEXT_EXT_COMMON_REPORTS ?></legend>
<div class="cfg_listing">        
<ul id="reports_excluded_from_dashboard" class="sortable sortable-reports">
<?php
  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id  and r.reports_type='common' and r.id not in (" . $reports_list . ") order by e.name, r.name");
  while($v = db_fetch_array($reports_query))
  {
    echo '<li id="report_' . $v['id'] . '"><div>' . $v['name'] . '</div></li>';
  }
...
<div class="cfg_listing">        
  ...
  <?php
  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id  and r.reports_type='common' and r.id in (" . $counters_list . ") order by field(r.id,{$counters_list})");
  while($v = db_fetch_array($reports_query))
  {
    echo '<li id="report_' . $v['id'] . '"><div>' . $v['name'] . '</div></li>';
  }
...
<?php
  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id  and r.reports_type='common' and r.id not in (" . $counters_list . ") order by e.name, r.name");
  while($v = db_fetch_array($reports_query))
  {
    echo '<li id="report_' . $v['id'] . '"><div>' . $v['name'] . '</div></li>';
  }
