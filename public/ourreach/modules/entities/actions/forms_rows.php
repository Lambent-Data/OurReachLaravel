...
switch($app_module_action)
{
    case 'save':
        ...
        if(isset($_GET['id']))
        {
            ...
            for($i=($_POST['columns']+1); $i<=4;$i++)
            {                
                db_query("update app_fields set forms_rows_position='' where forms_rows_position='" . _GET('id') . ":" . $i . "' and entities_id='" . _GET('entities_id'). "'");
            }            
        }
        else
        {   
            $check_query = db_query("select (max(sort_order)+1) as total from app_forms_rows where entities_id='" . _GET('entities_id'). "' and forms_tabs_id='" . _GET('forms_tabs_id'). "'");
    ...
    case 'sort_rows':
        ...
        while($tabs = db_fetch_array($tabs_query))
        {
            if(isset($_POST['forms_rows_' . $tabs['id']]))
            {
                ...
                {
                    db_perform('app_forms_rows',array('sort_order'=>$sort_order,'forms_tabs_id'=>$tabs['id']),'update',"id='" . db_input($v) . "'");
                    $sort_order++;
                    
                    db_query("update app_fields set forms_tabs_id='" . $tabs['id'] . "' where entities_id='" . _GET('entities_id'). "' and forms_rows_position in ('" . $v . ":1','" . $v . ":2','" . $v . ":3','" . $v . ":4')");
                }
            }
        }
        exit();
    ...
    case 'delete':
        ...
        if(isset($_GET['id']))
        {
            ...
            $v = _GET('id');
            db_query("update app_fields set forms_rows_position='' where entities_id='" . _GET('entities_id'). "' and forms_rows_position in ('" . $v . ":1','" . $v . ":2','" . $v . ":3','" . $v . ":4')");
        }
